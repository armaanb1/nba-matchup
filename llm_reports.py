...
SYSTEM_PROMPT = (
    ...
    "  Offensive archetypes: Shot Creator, Primary Ball Handler, Secondary Ball "
    "Handler, Slasher, Off Screen Shooter, Movement Shooter, Stationary Shooter, "
    "Athletic Finisher, Versatile Big, Stretch Big, Post Scorer, Roll & Cut Big.\n"
    "  Defensive roles: Point of Attack, Wing Stopper, Chaser, Helper, "
    "Anchor Big, Mobile Big, Low Activity.\n"
    "Example: 'Point of Attack defenders contain Brunson most effectively — "
    "Jalen Suggs (0.462 PPP, 97 possessions) is the clearest example.' "
    "Not: 'long, physical guards who navigate screens well.' Use the label."
...
""".strip()
    ...
    if role == "defense":
        similarity_paragraphs = (
            f"Paragraph 2 — DEFENSIVE SIMILARITY: What the comparable defenders share "
            ...
            f"Paragraph 3 — COVERAGE TENDENCIES: Across the comparable group, which "
            f"coverage assignments and help rotations are consistent — and where does "
            f"each player diverge from {target.name}'s scheme. Use shared opponent data "
            f"to explain which offensive archetypes this group collectively struggles "
            f"with and which they consistently contain.\n\n"
        )
        p1_desc = (
            f"Paragraph 1 — DEFENSIVE ARCHETYPE: What defensive archetype does "
            f"{target.name} represent. What physical and schematic traits define how "
            f"they guard — coverage responsibilities, floor zones they protect, help "
            f"tendencies, and which offensive archetypes they consistently contain or "
            f"struggle against. This is the baseline — everything in the report "
            f"connects back to it.\n\n"
        )
    else:
        similarity_paragraphs = (
            ...
            f"Paragraph 3 — DEFENSIVE EXPOSURE: Across the comparable group, which "
            f"defensive coverages and defender archetypes consistently suppress their "
            f"scoring — and where each player's offensive game diverges from "
            f"{target.name}'s. Use the shared defender data to explain which defensive "
            f"assignments neutralize this offensive profile.\n\n"
        )
        p1_desc = (
            f"Paragraph 1 — OFFENSIVE ARCHETYPE: What offensive archetype does "
            f"{target.name} represent. What physical and schematic traits define how "
            f"they score — floor zones, creation methods, defender archetypes they "
            f"exploit. This is the baseline — everything in the report connects back "
            f"to it.\n\n"
        )
    ...
    prompt = (
        ...
        f"to them. The {n} most similar players by MPS score are: {similar_names}. "
        f"Reference these players by name throughout the report. Four paragraphs.\n\n"
        f"{p1_desc}"
        f"{similarity_paragraphs}"
        ...
        f"replacement from the {n} similar players and the specific reason "
